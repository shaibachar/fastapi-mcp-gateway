...
async def create_mcp_server_from_graphql(
    graphql_url: str,
    config: Optional[GatewayConfig] = None,
) -> "MCPGraphQLServer":
    """Create an MCP server from a GraphQL endpoint.

    Args:
        graphql_url: URL to the GraphQL endpoint
        config: Optional gateway configuration

    Returns:
        MCPGraphQLServer instance
    ...
    # Load schema from GraphQL endpoint via introspection
    schema = await GraphQLLoader.load_from_url(graphql_url)

    # Create config if not provided, ensuring GRAPHQL mode
    if config is None:
        config = GatewayConfig(mode=ExecutionMode.GRAPHQL, base_url=graphql_url)

    return MCPGraphQLServer(schema=schema, config=config, endpoint_url=graphql_url)


def create_mcp_server_from_graphql_schema(
    schema_file: str,
    endpoint_url: str,
    config: Optional[GatewayConfig] = None,
) -> "MCPGraphQLServer":
    """Create an MCP server from a GraphQL schema file.

    Args:
        schema_file: Path to the GraphQL schema file (.graphql or .gql)
        endpoint_url: URL to the GraphQL endpoint for execution
        config: Optional gateway configuration
    ...
    Example:
        ```python
        from fastapi_mcp_gateway import create_mcp_server_from_graphql_schema

        mcp_server = create_mcp_server_from_graphql_schema(
            "schema.graphql",
            "http://localhost:8000/graphql"
        )
        await mcp_server.run()
    ...
    schema = GraphQLLoader.load_from_file(schema_file)

    # Create config if not provided, ensuring GRAPHQL mode
    if config is None:
        config = GatewayConfig(mode=ExecutionMode.GRAPHQL, base_url=endpoint_url)

    return MCPGraphQLServer(schema=schema, config=config, endpoint_url=endpoint_url)


class MCPGraphQLServer:
    """MCP server that exposes GraphQL operations as tools."""
    ...
    def __init__(
        ...
        """Initialize the MCP GraphQL server.
        ...
        """
        self.schema = schema
        self.config = config or GatewayConfig(mode=ExecutionMode.GRAPHQL)
        self.endpoint_url = endpoint_url or self.config.base_url
        ...
        # Create executor
        from fastapi_mcp_gateway.graphql_execution import GraphQLExecutor
        ...
        # Create MCP server
        self.server = Server("fastapi-mcp-gateway-graphql")
        self._register_handlers()

        logger.info(f"MCPGraphQLServer initialized with {len(self.tools)} tools")
    ...
    def _create_tools(self) -> list[dict[str, Any]]:
        ...
        include_queries = self.config.graphql_operation_filter in [
            GraphQLOperationFilter.ALL,
            GraphQLOperationFilter.QUERIES_ONLY,
        ]
        include_mutations = self.config.graphql_operation_filter in [
            GraphQLOperationFilter.ALL,
            GraphQLOperationFilter.MUTATIONS_ONLY,
        ]

        all_tools = GraphQLOperationMapper.map_graphql_to_tools(
            self.schema,
        ...
        filtered_tools = [
            tool
            for tool in all_tools
            if self.config.should_include_operation(tool["_metadata"]["operation_name"])
        ]
